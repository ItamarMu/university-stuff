...
//Error, warning and info messages should be printed at info level
...
//Warning and info messages should be printed at warning level
...
static bool spLoggerPrintMsgTest() {
	...
	ASSERT_TRUE(spLoggerCreate(testFile,SP_LOGGER_WARNING_ERROR_LEVEL) == SP_LOGGER_SUCCESS);
	ASSERT_TRUE(spLoggerPrintMsg("HELLO"));
	spLoggerDestroy();
	ASSERT_TRUE(identicalFiles(testFile,expectedFile));
	return true;
}
